...
const TestimonialCard = ({ imgSrc, description, titleText, titleNumber }: TestimonialCardProps) => {
  ...
  return (
    <Card
      ...
      className="flex flex-col sm:flex-row items-center sm:items-start gap-4 p-4 w-full rounded-xl bg-[#F0FAFF] shadow-md border-none transition-all max-w-[400px]"
    >
      {/* Icon */}
      <div className="w-full sm:w-[30%] flex justify-center sm:justify-start">
        <div className="flex items-center justify-center size-24 sm:size-28 bg-white rounded-lg">
          <span className={cn('text-5xl sm:text-6xl transition-transform duration-300', isCardHovered && "scale-125")}>
            {imgSrc}
          </span>
        </div>
      </div>
